        ...
        if(e){
            ...
            if(findings.error){
                ...
                for(var p = 0;p<findings.content.length;p++){
                    ...
                    d.innerHTML +='<div class="featured-investments-logo"><img height="100%" src="images/iibanc resources/icons/ziggurat  watermark.svg"/></div><div class="featured-investments-iconinfo"><h4 style="margin:0px;">' + findings.content[p].profile.compname + '</h4><p style="margin:0px;">email: '+ findings.content[p].email + '<br/>bio: '+findings.content[p].profile.description + '</p><span>Seeking: $' + parseInt(findings.content[p].profile.amount).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +' | Raised: $' + parseInt(findings.content[p].profile.raised).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +'</span></br><div style="height:6px;width:100%;margin: 3px auto;background-color:red;border-radius:3px;"><div id="investedamountbar" style="height:100%;width:'+ amountpercent+'%;background-color:green;border-radius:3px;"></div></div><p style="font-size:9px;margin:0px auto;">Created: '+weekday[new Date(pooop).getDay()] +':- '+ Month[new Date(pooop).getMonth()] +', '+ new Date(pooop).getDate()+',  '+ new Date(pooop).getFullYear() +'</p></div>';
                    //console.log(divele)
...
//doc.addEventListener("DOMContentLoaded",function(event){
    ...
    */
    ...
    doc.querySelector('body').addEventListener("click", (ev)=>{
        ...
        if(ev.target.getAttribute("id") == "bufferinvsclick" || ev.target.getAttribute("id") =="investedamountbar"){
